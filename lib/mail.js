const nodemailer = require("nodemailer");
var smtpTransport = require("nodemailer-smtp-transport");

function sendEmail(title, txt, mailTo) {
  var transporter = nodemailer.createTransport({
    host: "smtp.naver.com", //SMTP 서버 주소
    secure: true, //보안 서버 사용 false로 적용시 port 옵션 추가 필요
    auth: {
<<<<<<< HEAD
      user: 'sugarmanwork', //메일서버 계정
      pass: 'sugarman**2120@!' //메일서버 비번
=======
      user: "sugarmanwork@naver.com", //메일서버 계정
      pass: "sugarman2#", //메일서버 비번
>>>>>>> c386b54612ac70f0845f3c92ad171d73383100cc
    },
    tls: {
      rejectUnauthorized: false,
    },
  });

  var mailOptions = {
<<<<<<< HEAD
    from: 'sugarmanwork@naver.com', //보내는 사람 주소
    to: mailTo ? mailTo : 'sugarmanwork@gmail.com', //받는 사람 주소
=======
    from: "sugarmanwork@naver.com", //보내는 사람 주소
    to: "sugarmanwork@gmail.com", //받는 사람 주소
>>>>>>> c386b54612ac70f0845f3c92ad171d73383100cc
    subject: title, //제목
    html: txt,
  };

  //전송 시작!
  transporter.sendMail(mailOptions, function (error, info) {
    if (error) {
      //에러
      console.log("에러남....", error);
    }
    //전송 완료
    console.log("되는건가");
    transporter.close();
  });
}

module.exports = sendEmail;
